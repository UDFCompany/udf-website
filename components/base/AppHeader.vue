<template>
  <nav class="fixed p-4 w-full transition-all duration-300 z-50" :class="{
      ...
      <div class="flex items-center justify-between">
        ...
        <NuxtLinkLocale to="/">
          <div class="flex items-center">
            <img src="/udf_logo.png" alt="Logo" class="h-8 w-8 mr-2" referrerpolicy="no-referrer" />
            <span class="transition-colors hover:text-opacity-80 md:text-xl font-bold"
              :class="{ 'text-white': !scrolled, 'text-gray-800': scrolled }">{{ t("companyName") }}</span>
          </div>
        </NuxtLinkLocale>

        <!-- Desktop Navigation -->
        <div class="hidden md:flex items-center space-x-6">
          <NuxtLinkLocale to="/" class="transition-colors hover:text-opacity-80 px-3 py-1 rounded-md"
            :class="[scrolled ? 'text-gray-800' : 'text-white']" active-class="font-bold underline underline-offset-8">
            {{ t("home") }}
          </NuxtLinkLocale>
          <div class="relative group">
            <NuxtLinkLocale to="/products" class="transition-colors hover:text-opacity-80" :class="[
              ...
              <button class="transition-colors hover:text-opacity-80 flex items-center"
                :class="scrolled ? 'text-gray-800' : 'text-white'" @mouseover="productDropdown = true"
            ...
            <!-- Dropdown Menu -->
            <div class="absolute hidden group-hover:block w-72 bg-white shadow-lg py-2 rounded-lg"
              v-show="productDropdown" @mouseleave="productDropdown = false" @mouseenter="productDropdown = true"
          ...
          <NuxtLinkLocale to="/about" class="transition-colors hover:text-opacity-80"
            :class="[scrolled ? 'text-gray-800' : 'text-white']" active-class="font-bold underline underline-offset-8">
            {{ t("about") }}
          </NuxtLinkLocale>
          <NuxtLinkLocale to="/contact" class="transition-colors hover:text-opacity-80"
            :class="[scrolled ? 'text-gray-800' : 'text-white']" active-class="font-bold underline underline-offset-8">
            {{ t("contact") }}
          </NuxtLinkLocale>
          ...
          <div class="flex items-center space-x-2 ml-4">
            <button @click="changeLanguage('vi')" :class="[
              ...
              , scrolled ? 'text-gray-800' : 'text-white']" class="px-2 py-1 rounded transition-colors">
              VN
            </button>
            <span :class="{
              'text-white': !scrolled,
      ...
      <transition enter-active-class="transition-all duration-300 ease-out"
        ...
        <div v-show="mobileMenu"
          ...
          <div class="container mx-auto px-4">
            <NuxtLinkLocale to="/" class="block py-3 border-b" :class="[
              scrolled
                ? 'text-gray-800 border-gray-100'
                : 'text-white border-orange-400',
            ]" active-class="font-bold" @click="closeMenus">
              {{ t("home") }}
            </NuxtLinkLocale>
            <div class="relative">
              <div class="flex items-center justify-between">
                <NuxtLinkLocale to="/products" class="w-full text-left py-3 border-b flex justify-between items-center"
                  :class="[
                    scrolled
                      ? 'text-gray-800 border-gray-100'
                      : 'text-white border-orange-400',
                ...
                <button @click="productDropdown = !productDropdown"
                  class="text-center p-2 border-b flex justify-between items-center" :class="[
                    scrolled
                      ? 'text-gray-800 border-gray-100'
                      : 'text-white border-orange-400',
              ...
              <transition enter-active-class="transition-all duration-300 ease-out"
                ...
                <div v-show="productDropdown" :class="[scrolled ? 'bg-gray-50' : 'bg-orange-600']" class="px-4">
                  <NuxtLinkLocale v-for="product in products" :to="product.link" class="block py-3 border-b" :class="[
                    scrolled
                      ? 'text-gray-800 border-gray-100'
                      : 'text-white border-orange-400',
                  ]" active-class="font-bold" @click="closeMenus" :key="product.id">
                    {{ product.name }}
                  </NuxtLinkLocale>
                </div>
              </transition>
            ...
            <NuxtLinkLocale to="/about" class="block py-3 border-b" :class="[
              scrolled
                ? 'text-gray-800 border-gray-100'
                : 'text-white border-orange-400',
            ]" active-class="font-bold" @click="closeMenus">
            ...
            <NuxtLinkLocale to="/contact" class="block py-3 border-b" :class="[
              scrolled
                ? 'text-gray-800 border-gray-100'
                : 'text-white border-orange-400',
            ]" active-class="font-bold" @click="closeMenus">
